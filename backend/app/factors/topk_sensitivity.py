...
口径见 `engine/cost_curves` 模块头：
  ...
  · ⚠ **简化估算：买入端已按开关剔除 T/T+1 涨停、T+1 停牌样本；未模拟跌停（卖不出）与流动性冲击**。
...
BASE_KS = (1, 2, 3, 5, 10, 20, 50, 100, 200)  # 固定候选（只数）；另加 10%/20% 两档
# ⚠ 与前端 K 预设（`SingleFactorTestPanel.K_PRESETS` = 1/2/3/5/10/20/50/100/10%/20%）保持
#   **超集**关系：预设里每个整数都在本表里（K=2 于 2026-09-12 用户要求补上）⇒ 表与曲线档位对得上。
NOTE = ("简化估算：买入端已按剔除开关剔除 T/T+1 涨停、T+1 停牌样本，未模拟跌停（卖不出）与流动性冲击；"
        "调仓期默认 = 预测周期 h（按预测周期调仓换股），"
        "成本按调仓期扣、只扣实际调仓的股票；首期建仓不计费；持仓只数 1/3 仅供边界参考。"
        "⚠ 本表按固定持仓只数（同一方向：超额收益最强分位组所在的那一端）计算；"
        "默认档（最强分位组）是逐日等分、只数逐日变化，与固定只数略有差异（实测成员中位差 4 只）。"
        "⚠ 「每期毛收益」是 label（h 期前视）口径的每期累加毛收益，"
        "不是可年化的真实收益 ⇒ 「成本吞噬」只作量级参考，须与曲线同口径解读。")
