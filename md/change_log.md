...
- **背景**（用户提问）：「大公式第一次显示"计算中"，我关闭之后，打开别的公式、再重新打开大公式，
  ...
  （前端缓存随组件卸载丢失 ✓）。
- **实现**（方案：复用 v1.2.5 的 `cancel_cb` 挂点 ✓，不新增线程/进程机制 ✓）：
...
- ⚠ 仍保留 v1.2.5 的"同事件指纹被更新请求顶掉"机制 ✓（两条互补：**关弹窗** / **改参数重算** ✓）。
...
- **根因（我的疏漏 ✗）**：请求 effect 的依赖数组**缺 `navK` / `navCost` / `sourceTaskId`** ✗ ——
  v1.2.9 是"一律手动"时期只留 `navTick` ✓，v1.2.12 修复时**只补回了 `result`** ✗
...
- **用户指正**：「**二浪加强点进去怎么还有计算按钮？不应该先自动计算，然后根据实际判断
  要不要显示重新计算按钮么？**」✓ **完全正确** ✓。
- **根因**：v1.2.11 把按钮做成了**常显** ✗ ⇒ 快公式（自动算完 ✓）也一直挂着一个「计算」按钮 ✓，
  语义上等于"还没算过"，误导 ✓。
- **修法**：
...
- **修法**：
  ...
  2. 请求 effect 依赖补回 **`result`** ✓ ⇒ 换公式也走一次自动判定 ✓
     （v1.2.9 只留 `navTick` 是"一律手动"时期的写法 ✓ 现在自动识别必须加回 ✓）。
...
- **用户给出的真实耗时**：`取价 17.64s + 回测 57.09s` ✓ ⇒ **验证了 v1.2.6 的量化分析** ✓：
  实验室样本（4000 事件）1.28s ✓ ⇒ 「过顶」**14.88 万事件 = 37 倍** ⇒ 预期 ~47s ⇒ 实测 57s ✓
  ⇒ **瓶颈确为"逐笔开销 × 笔数"** ✓（优化前约 98s，v1.2.7/8 后 −42% ✓）。
...
- 背景（v1.2.6 量化 ✓）：`_limit` 被调 **14.9 万次** + `_suspended` 7.5 万次 ⇒ 合计 ≈15% 总耗时 ✗
...
- **量化先行（v1.2.6）**：新增 `ai_test/bench_bt_big.py`（**1500 日 × 300 只 × 4000 事件** + `cProfile`）。
