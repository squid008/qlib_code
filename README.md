...
> **当前版本：v1.20.73**（语义化版本，后端 `backend/app/__init__.py` 定义，前端标题栏显示）
>
> ⚠ **版本号断代（2026-09-18）**：此前为 `1.19.x` 线性递增；`1.20.0` 起改为**大改动锚点**
> （⚠ **必须是 `1.20.x` 而非 `1.2.x`** —— 语义化版本里 `1.2.x < 1.19.x`，会让回退找错位置 ✗）。
> 回退时以 **`v1.20.0`** 为分界：其前为旧语义，其后为新语义（前复权真落地 / 公式编辑器修复 / 回测提速）。
>
> ★ **递增规则（2026-09-25 定稿，照办 ✓）**：**按位进位** —— 补丁号到 `99` 向次版本进位
> （`1.20.99 → 1.21.0`）、次版本到 `99` 向主版本进位（`1.99.99 → 2.0.0`）；
> **除人明确指定版本号外，不许无规则跳版本** ✗（别从 `1.20.71` 直接跳 `1.21.0` ✗）。
> 一键递增：`python scripts/bump_version.py`（加 `--dry-run` 只看结果 ✓；显式指定则传版本号，如 `1.21.0`）。
>
...
4. **复权数据口径（v1.6.1 修正；v1.19.87/97 + v1.20.0 补充，务必知晓）**：本机 qlib 数据的 `$close/$open/$high/$low` 原生即**后复权价** = 真实价 × `$factor`
   ...
   - ⚠ **`$market_cap` 与复权无关**（市值 = 真实价 × 总股本，复权不动股本 ✓）⇒ `PRICE_FIELDS`
     **不含**它 ✓，三种复权模式下**恒等透传** ✓（实测前后复权 K=20 逐位相同 ✓）。切勿手写
     `$market_cap*$factor` ✗（那是人造口径）。
   - ⚠ **默认复权已统一为「后复权」**（v1.19.97 ✓ 单因子 / 事件研究 / 训练 / 回测）；`label`（收益率）
     在非 `none` 模式下**强制走后复权** ✓（避免除权日跳空被当成下跌 ✗）。
   - ⚠ **单因子测试接口不做字段别名编译** ✗（原样透传 `expression` ✓）⇒ 独立脚本/接口必须写
     `$market_cap` ✓；写 `MARKET_CAP` 会整列 NaN 并被判成 `is_binary` 0/1 信号 ✗（前端「编译并保存」正常 ✓）。；`$change` 是真实价（东财"不复权"口径）涨跌幅。三种复权取价：
...
| `md/formulas_sync.md` | **公式库（自定义公式）多地同步口径**：按装机分文件、只增不减+时间戳合并、pull/push 语义与 `scripts/formula_sync.py` 用法 |
