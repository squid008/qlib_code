...
> **当前版本：v1.2.16**（语义化版本，后端 `backend/app/__init__.py` 定义，前端标题栏显示）
>
> ⚠ **版本号断代（2026-09-18）**：此前为 `1.19.x` 线性递增；`1.2.0` 起改为**大改动锚点**
> （用户要求"后面我回退就知道回退到哪里"）⇒ `1.19.98 ~ 1.19.103` 是 `1.2.0` 的**预备提交**。
> 回退时以 **`v1.2.0`** 为分界：其前为旧语义，其后为新语义（前复权真落地 / 公式编辑器修复 / 回测提速）。
>
...
4. **复权数据口径（v1.6.1 修正；v1.19.87/97 + v1.2.0 补充，务必知晓）**：本机 qlib 数据的 `$close/$open/$high/$low` 原生即**后复权价** = 真实价 × `$factor`
